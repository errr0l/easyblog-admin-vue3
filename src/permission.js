import router, { asyncRoutes, NOT_FOUND, resetRouter } from './router';
...
// 不需要通过接口获取的路径
const whiteList = ['/login', '/oauth2/callback', '/404', '/register'];
let appStore;
// 创建用户菜单【404页面需要在最后添加】
...
router.beforeEach(async (to, from, next) => {
    ...
    // 获取配置信息，并初始化【主要是用户和令牌信息】
    if (!appStore.config) {
        const resp = await fetch(configUrl)
            .then(res => res.json());
        // store.commit('app/SET_CONFIG', { config: resp });
        appStore.setConfig({ config: resp });
        // store.commit('user/INIT', { prefix: resp.PREFIX });
    ...
    // 因此此处不需要添加权限判断逻辑；
    // 从login跳转到/，会连续请求两次获取菜单接口
    const _userRoutes = appStore.permissionStore.userRoutes;
    if (_userRoutes && _userRoutes.length) {
        next();
        return;
    }

    // 没有菜单时【比如，刷新了页面】，请求菜单权限
    const resp = await getAccountPermissionList();
    if (!resp || resp.code !== 0) {
        throw new Error("获取菜单失败");
    }

    const userRoutes = makeUserRoutes(asyncRoutes, resp.data.filter(item => item.type === PERMISSION_MENU));
    ...
    appStore.permissionStore.setRoutes(userRoutes);
